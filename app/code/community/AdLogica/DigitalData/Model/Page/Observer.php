...
class AdLogica_DigitalData_Model_Page_Observer {

  // This is the DitialData specification Version
  // Add Spec URL
 ...
 */
  ...
  public function _setPage() {
    ...
    //          Please migrate any frontend JavaScripts using this `digital_data.page.category` variable
  ...
  public function _getProductModel($product) {
    ...
    $product_model['price']['unit_price']      = (float) $product->getPrice();
    $product_model['price']['unit_sale_price'] = (float) $product->getFinalPrice();
    ...
    $product_model['productInfo']['stock']           = (int) $this->_getProductStock($product);
    ...
    if (isset($categories[1])) {
      $product_model['category']['subcategory'] = $categories[1];
    }
    
    $product_model['productInfo']['productImage'] = $product->getMediaConfig()->getMediaUrl($product->getData('image'));
  ...
  public function _getLineItems($items, $page_type) {
	...
	$litem_product = $this->_getProductModel($product);
        $litem_model             = array();
        $litem_model['productInfo']  = $litem_product['productInfo'];
	$litem_model['category']  = $litem_product['category'];
        $litem_model['price']  = $litem_product['price'];
        ...
        // Recalculate unit_sale_price after voucher 
        ...
        $litem_model['price']['unit_sale_price'] = $unit_sale_price_after_discount;
  ...
  public function _setListing() {
    ...
    if ($this->_isCategory()) {
      ...
      $event["eventInfo"]["eventName"] = "User Search";
      $event["eventInfo"]["type"] = "structured";
      $event["category"] = array();
      $event["category"]["primaryCategory"] = "Ecomm";
      $event["attributes"] = array();
      $event["attributes"]["property"] = "keywords";
      $event["attributes"]["value"] = $_GET['q'];
  ...
  public function _setTranscation() {
    ...
    if ($orderId) {
      ...
      $transaction['total']['subtotal']             = (float) $order->getSubtotal();
      $transaction['total']['tax']                  = (float) $order->getTaxAmount();
      $transaction['total']['subtotal_include_tax'] = $this->_doesSubtotalIncludeTax($order, $transaction['tax']);
      ...
      $transaction['total']['shipping_cost']   = (float) $order->getShippingAmount();
      ...
      $transaction['profile']['address'] = $this->_getAddress($shippingAddress);
  ...
  public function setDigitalData($observer) {
    $this->_setUser();
    $this->_setPage();
